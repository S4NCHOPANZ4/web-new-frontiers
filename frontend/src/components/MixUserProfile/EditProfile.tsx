import { CiEdit } from "react-icons/ci";
import styles from "../../styles";

const EditProfile = () => {
    return (
        <div>
            <p className="font-semibold text-xl text-center text_linearGradient_ver3">Editar Perfil</p>
            <div className="my-5 flex items-center">
                <img className="h-[100px] w-[100px] rounded-full" src={'data:image/jpeg;base64,'} alt="profile page" />
                <div className="mx-3">
                    <p className="font-bold text-lg">Foto de perfil</p>
                    <p className="text-sm flex items-center hover:underline cursor-pointer">Cambia tu foto de perfil <span className="mx-1"><CiEdit /></span></p>
                </div>
            </div>
            <div className="flex flex-col space-y-2">
                <div>
                    <p className="font-semibold tracking-tight">Nombre de usuario</p>
                    <input placeholder="Username" className={`${styles.input} w-full text-sm`} />
                </div>
                <div>
                    <p className="font-semibold tracking-tight">Correo Electronico</p>
                    <input placeholder="Email" className={`${styles.input} w-full text-sm`} />
                </div>
            </div>
            <p className="font-semibold text-md pt-3 pb-1 border-b text-end text_linearGradient_ver3">Datos Personales</p>
            <div className="flex flex-col space-y-2 my-2">
                <div className="flex justify-evenly space-x-2">
                    <div className="w-full">
                        <p className="font-semibold tracking-tight">Nombre</p>
                        <input placeholder="Nombre" className={`${styles.input} w-full text-sm`} />
                    </div>
                    <div className="w-full">
                        <p className="font-semibold tracking-tight">Apellido</p>
                        <input placeholder="Apellido" className={`${styles.input} w-full text-sm`} />
                    </div>
                </div>
                <div className="flex justify-evenly space-x-2">
                    <div className="w-full">
                        <p className="font-semibold tracking-tight">Teléfono</p>
                        <div className="flex">
                            <input placeholder="+57" className={`${styles.input} text-sm w-[50px] noExtraInputUI`} type="number"/>
                            <input placeholder="Numero Telefónico" className={`${styles.input} w-full text-sm noExtraInputUI`} type="number"/>
                        </div>
                    </div>
                    <div className="w-full">
                        <p className="font-semibold tracking-tight">Correo Electronico</p>
                        <input placeholder="Email" className={`${styles.input} w-full text-sm `} />
                    </div>
                </div>
                <div>
                    <p className="font-semibold tracking-tight">País</p>
                    <input placeholder="País" className={`${styles.input} w-full text-sm`} />
                </div>
                <div className="flex items-center justify-end w-full">
                    <button className="text-white font-semibold linearGradient_ver3 px-2 py-1 rounded-sm text-sm">Guardar</button>
                </div>
            </div>

        </div>
    )
}

export default EditProfile